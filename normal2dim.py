...
def plot(data,centroids):
        ...
        plt.ylabel('Power')
        plt.savefig('normalized2dim.png')
...
for jd in json_data:
	...
	if(len(power_list) == 0):
		power_list = [0]
	if((jd['job']['app_name'] == "gaussian" or (jd['job']['app_name'] == "mono" and monocount < 40) or (jd['job']['app_name'] == "python" and pythoncount<30)) and (converttime(jd['job']['cpu_used']) != 0)):
		...
		if(jd['job']['app_name'] == "python"):
			pythoncount+=1
		if(jd['job']['app_name'] == "mono"):
			monocount+=1
del json_data
gc.collect()
...
del data2
gc.collect()
df = df[df.wallclock_used < 7599]
df = df[df.power != 0]
df = df.dropna()
df2 = df2[df2.wallclock_used < 7599]
df2 = df2[df2.power != 0]
df2 = df2.dropna()
centroids = kmeans()
print("Normalized 2-Dimension Clustering\n----------------------\nClusters:\n", centroids.cluster_centers_)
