...
def plotnamed(data,centroids):
        ...
        plt.ylabel('Power')
        plt.savefig('7named.png')
       
def plot(data,centroids):
        color_dict = ['r', 'y', 'b']
        x = data['wallclock_used']/data['nodes_used']
        y = data['power']
        plt.scatter(x, y, color =[color_dict[i] for i in data['Assigned_Clus']])
        plt.xlabel('Run_Time / Nodes')
        plt.ylabel('Power')
        plt.savefig('7dim.png')
...
with open('10k.anon.json', 'rb') as f:
	for line in f:
		if(c <= LIMIT):
			...
			break
data = {'power':[], 'run_time':[], 'nodes_used':[], 'processors_used':[], 'wallclock_used':[], 'cpu_used':[], 'mem_used':[]}#, 'app_name':[], 'cpu_over_run':[]}
data2 = {'power':[], 'run_time':[], 'nodes_used':[], 'processors_used':[], 'wallclock_used':[], 'cpu_used':[], 'app_name':[], 'mem_used':[]}#, 'app_name':[]}
for jd in json_data:
	power_list = []
	for p in jd['power']:
		if(p is None):
	...
	if(len(power_list) == 0):
		power_list = [0]
	if(jd['job']['app_name'] == "gaussian" or jd['job']['app_name'] == "mono" or jd['job']['app_name'] == "python"):
		data['power'].append(np.average(power_list))
		data2['power'].append(np.average(power_list))
		t2 = jd['job']['end_time']
		t1 = jd['job']['start_time']
		data['run_time'].append((datetime.strptime(t2, FMT) - datetime.strptime(t1, FMT)).total_seconds())
		...
		data2['run_time'].append((datetime.strptime(t2, FMT) - datetime.strptime(t1, FMT)).total_seconds())
		data2['nodes_used'].append(jd['job']['nodes_used'])
		data2['processors_used'].append(jd['job']['processors_used'])
		data2['wallclock_used'].append(converttime(jd['job']['wallclock_used']))
		data2['cpu_used'].append(converttime(jd['job']['cpu_used']))
		data2['mem_used'].append(jd['job']['mem_used'])
...
print("SIZE OF DF2: ", len(df2.index))
