...
with open('10k.anon.json', 'rb') as f:
	for line in f:
		if(c <= LIMIT):
			...
			break
data = {'power':[], 'nodes_used':[], 'processors_used':[], 'wallclock_used':[], 'cpu_used':[]}
data2 = {'power':[], 'nodes_used':[], 'processors_used':[], 'wallclock_used':[], 'cpu_used':[], 'app_name': []}
for jd in json_data:
	power_list = []
	for p in jd['power']:
		if(p is None):
